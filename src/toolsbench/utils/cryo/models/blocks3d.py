...
from .se3d import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
import math
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        if positional_encoding:
            self.positional_encoding = SinusoidalPositionEmbeddings(out_channels)
    ...
    def forward(self, x, t=None):
        ...
        if self.use_pe:
            # print(x.shape)
            # enc = self.positional_encoding(t)
            # print(enc.shape)
            x = x + self.positional_encoding(t)[:, :, None, None, None]
        return x
